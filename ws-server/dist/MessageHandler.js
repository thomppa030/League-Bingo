...
export class MessageHandler {
    connectionManager;
    sessionValidator;
    constructor(connectionManager, sessionValidator) {
    ...
    async handlePlayerUpdate(ws, message) {
        if (!ws.sessionId)
            return;
    ...
    async handleSquareClaim(ws, message) {
    ...
    async handleSquareConfirmation(ws, message) {
        if (!ws.sessionId)
            return;
    ...
    async handleSquareRejection(ws, message) {
        if (!ws.sessionId)
            return;
        // Validate that player is GM
        const isGM = await this.sessionValidator.isGameMaster(ws.sessionId, ws.playerId);
